...
object AlertsServer extends IOApp {
  ...
  override def run(args: List[String]): IO[ExitCode] = {
    for {
      ...
      usersService <- IO( new UserService(userRepository, credentialsRepository, applecredentialsRepository, fbcredentialsRepository, refreshTokensRepo, alertsRepository, feedbackRepository, secrets.read.surfsUp.facebook.key, androidPublisher, applePrivateKey, emailSender))
