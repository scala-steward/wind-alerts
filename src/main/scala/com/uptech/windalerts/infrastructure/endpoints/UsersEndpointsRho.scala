...
import com.uptech.windalerts.domain.{BeachNotFoundError, SurfsUpError, UserNotFoundError, statics}
...
class UsersEndpointsRho[F[+_] : Effect]
(repos: Repos[F], userCredentialsService: UserCredentialService[F], userService: UserService[F], socialLoginService: SocialLoginService[F], userRolesService: UserRolesService[F], subscriptionsService: SubscriptionsService[F], httpErrorHandler: HttpErrorHandler[F]) extends RhoRoutes[F] {
