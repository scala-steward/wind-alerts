...
class AndroidSubscription[F[_] : Sync](androidPublisher: AndroidPublisher)(implicit F: Sync[F]) extends SocialSubscriptionProvider[F] {
  override def getPurchaseFromPlatform(token: String): F[SubscriptionPurchase] = {
    F.pure({
