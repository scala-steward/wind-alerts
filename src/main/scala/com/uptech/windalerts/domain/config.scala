...
object beaches {
  ...
  def read: Map[Long, Beach] = {
    val tryProd = Try(Source.fromFile("/app/resources/beaches-v2.json").getLines.mkString)
    val jsonContents = tryProd match {
      case Failure(_) => Source.fromFile("src/main/resources/beaches-v2.json").getLines.mkString
      case Success(_) => tryProd.get
    }
